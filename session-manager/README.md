...
| 话题行 | **这个对话里都讲了哪几件事** —— 抽出每一条有实质内容的真人发言开头, 编号排列, 始终一行。**点行=摊开全文并显示全部提问; 点某条前面的编号=直接跳到那一轮并展开回复** |
...
| **全文** | **把整个对话按「我的提问 → claude 的回复」摊开**, 两层折叠: 默认只显示最近 3 条提问, 点「显示全部 N 条提问」放开; 每条回复默认只给 3 行, 点「展开回复」看全文 |
| **建议** | **复盘这个对话**: 讲了什么 / 初衷是什么 / 达到没有 / 接着做什么。跑一次 `claude -p`(约 20-35 秒), 结果落盘, 再点直接出缓存 |
...
「全文」(`/api/tree`)把整个 jsonl 拆成轮次: **一条真人发言 + 它之后的全部 AI 回复 = 一轮**。

为什么是两层而不是全摊开: 一个长对话有一两百轮、单轮回复上万字, 一次铺出来就是一堵墙。
所以:

1. **第一层** 默认只留**最近 3 条提问** —— 最近的才是"我刚才在干嘛"。
   头上一行 `全文 · N 条提问` + 一个「显示全部 N 条提问」按钮。
2. **第二层** 每条回复只给 **3 行**(CSS `-webkit-line-clamp`), 点「展开回复」摊开全文,
   展开后限高 56vh 内部滚动, 免得一条回复把整页顶出去。

**三个入口, 同一棵树**(`openTree()`):

| 入口 | 打开成什么样 |
|---|---|
| `全文` 按钮 | 默认态 —— 最近 3 条提问, 回复各 3 行 |
| 点**话题行** | 直接显示全部提问 |
| 点话题前面的**编号** | 全部提问 + 滚到那一轮 + 自动展开那条回复, 高亮 1.6 秒 |

编号→轮次的对应不是按序号数的: 话题行滤掉了确认词、合并了连续追问、超过 14 条还会把中间
折叠掉, 所以话题 #12 通常不是第 12 轮。用**发言原文前缀**去 `turns` 里找(两边都做过同样的
空白折叠, 前缀比对必中)。实测: 一个 69 轮的对话, 话题行最后一条 → 正确跳到 `#69`。

**去掉了什么(2026-08-26)**: 原来的「看内容」按钮(最后 30 条消息流水)和话题行的
"点开变多行"都被全文完全覆盖, 而且是更差的版本(一个截 1200 字混排, 一个截 46 字还漏中段),
已删。`/api/transcript` 端点随之删掉。**踩到的坑**: 事件委托里原来就有一个
`else if(act === "topics")`, 新加的同名分支排在它后面, 永远走不到 —— 表现是"点话题行毫无反应",
而且不报错。加分支前先 `grep` 一遍 act 名。

三个实测出来的细节:

- **折叠时的 3 行只给"它说了什么", 不给工具流水。** 一轮回复常以十几个 `Bash`/`Read`
  开头, 照原样截 3 行会全是 `· Bash 读 xxx` —— 等于什么都没看见。所以摘要那份把
  `· ` 开头的行滤掉; 展开后工具行仍在, 但用 `--dim2` 弱化, 不抢正文。
- **子 agent(`isSidechain`)的往返不算一轮**, 只在所属轮次上记个条数(`N 条子 agent`)。
  混进主线会把"我问了什么"这条线冲散。
- **`hidden` 压不住 `.an`。** 摘要/全文两个 div 靠 `hidden` 互换, 而 `.an` 自己设了
  `display:-webkit-box`(line-clamp 需要), 会盖掉 `[hidden]` 的 UA 规则 —— 结果两份
  同时显示、正文重复了一遍。必须显式写 `.turn .an[hidden]{display:none}`。

超过 400 轮的对话只回传最近 400 轮, 单轮回复超过 6 万字截断, 两种情况页面上都会明说。
正在看全文时, 列表的 60 秒自动刷新会跳过这一轮 —— 否则重画会把展开的树冲掉。

**实测(2026-08-26)**: 最大的 33MB 对话(68 轮), 端到端 85ms / 回传 311KB;
27MB / 146 轮是 127ms / 572KB。⚠ 用 `localhost` 而不是 `127.0.0.1` 测的话会看到
恒定 ~2000ms —— 那是 Python 客户端先试 `::1` 的解析开销, 不是服务端, 所有端点都一样。

## 建议: 长对话的复盘(`advisor.py`)

长对话最常见的病不是找不到内容, 是**迷失重点** —— 开头想干 A, 二十轮之后在修 B 的边角,
而且当事人不会自己发现。「建议」就是把这件事显式问出来, 固定四段:

| 段 | 问的是 |
|---|---|
| 讲了什么 | 按时间顺序的实际经过, 点名文件/工具/数字 |
| 初衷 | **以最早几轮为准** —— 后来的收敛不是初衷 |
| 达到没有 | 先给判词(已达成/部分达成/未达成/已转向)再摆证据; 页面按判词染色 |
| 接着做什么 | 3-5 条可执行的下一步, 跑偏了就第一条说怎么拉回来 |

**语料构造**(`build_corpus`, 改了要同步改这里):

- 开头 `HEAD=8` 轮给全文 —— 初衷只在这里
- 结尾 `TAIL=20` 轮给全文 —— "现在卡在哪"只在这里
- 中间每轮只留提问的前 80 字, 保住脉络不付全文的钱
- **工具流水整行丢掉** —— 几百行 `· Bash …` 会把真正的判断句挤出窗口

**为什么不复用 titler**: 标题只看用户发言就够(标题是索引), 复盘必须看双方, 否则判不了
"达到没有"; 而且标题用 haiku(20 秒), 复盘要读几万字并下判断, 默认 `sonnet`
(`config.json` 里 `"advice_model"` 或环境变量 `SESSIONS_ADVICE_MODEL` 可换)。

**缓存**: 结果按对话追加进 `advice.jsonl`, 并记下**生成时对话有多少轮**。再点「建议」直接出
缓存(~1s); 之后又聊了几轮, 页头就显示"之后又聊了 N 轮, 这份是旧的", 要不要重算你自己点。
生成中列表的 60 秒自动刷新会暂停(重画会把子进程的进度条冲掉)。

**估时**: 生成期间按钮走秒(`复盘中 12s`), 框里报"已 Ns · 预计还要 Ms" —— 和 `↻ 重起标题`
一个规矩, 但**多带一个自变量**。耗时随对话长短差近一倍(实测 3 轮 18s / 71 轮 33s),
只存一个平均值对长短两头都是错的; 这里存 (轮数, 秒) 的样本, 最小二乘拟 `eta = a + b·轮数`。
样本 < 3 条、或样本的轮数没拉开(分母为 0)时退回均值 —— 那时它就退化成标题那套。
历史从 `advice.jsonl` 回传(`/api/advice` 的 `hist`), 换个浏览器也不用从零重学。

**实测(2026-08-26)**: 71 轮 / 33MB 的对话 → 语料 28k 字, 端到端 **34 秒**;
3 轮的小对话 18 秒。缓存命中 ~1s。三点样本 (3,18)(29,26)(71,33) 拟出 ≈0.22 秒/轮;
下一次 4 轮的实跑**预估 19s / 实际 17s**。

**踩过的坑**:

- **解析是唯一的失败面。** 第一次实跑就撞上模型返回不可解析。现在三层容错(剥 ```、
  括号配平取片段、裸换行/截断修补)+ 失败自动重跑一次 + 原始输出落到
  `advice_last_raw.txt`。回归测试 `test_advisor.py` 把见过的畸形输出全钉成用例,
  **并且带 4 条阴性用例** —— 一个只会说 yes 的解析器和没有解析器一样坏。
- **估时样本会把自己的回声吃进去。** 后端历史 + 本地历史是合并读的, 而"记一次新耗时"
  一开始写回的是**合并后**的那份 —— 于是每生成一次, 后端那几条就被再抄一遍进本地,
  拟合被自己的回声带偏。追加只能往本地那份写。
- **`claude -p` 会污染对话列表。** 它按 cwd 建一次性会话文件; `IGNORE_PROJ` 排掉的是
  `titler/` 的 slug, 而子进程原来继承的是 server 的 cwd —— 于是每起一个标题就往用户的
  列表里塞一条"给这个对话起标题"。发现时已积了 **17 条**。现在 `gen.py` / `advisor.py`
  都把 `cwd` 钉在 `titler/`, 历史遗留的那些按提示开头(`SELF_PROMPT_HEAD`)在列表里滤掉。
...
| 全文树(33MB / 68 轮的对话) | 85ms, 回传 311KB |
