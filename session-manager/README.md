...
WT 没有任何 CLI 能按标题聚焦标签(`wt focus-tab` 只认 index, 而 index 与对话没有映射),
但标签栏对 **UI Automation** 是可见的: 每个标签一个 TabItem, 名字就是标题, 拿
`SelectionItemPattern.Select()` 点它就瞬切 —— 不需要窗口在前台, 不发一个键盘事件。
实现在 `wt_tab.ps1`(顺手把全部标签名带回来, 重名检测零成本)。UIA 走不了(没有
powershell / 被策略禁了)才退回老办法: 提前台 + Ctrl+Tab 轮转(一格 0.18s, 至多 16 格,
发键盘前必须确认前台就是目标窗口, 焦点被抢走立刻停手)。

⚠ **ps1 落盘必须 UTF-8 带 BOM**: PowerShell 5.1 读无 BOM 的脚本按本机 ANSI 代码页解码,
中文注释会被啃成语法错误(实测: 报"意外的 }"), 而且是**静默地按错误编码读**, 不告诉你为什么。

**标签标题的权威来源是 `GetConsoleTitleW`, 不是记账。** 曾经轮转匹配用 hook 记的
`win_title`, 结果被污染了一整轮: hook 在 **Stop/Notification 时刻**也去抓窗口标题, 而 WT
窗口标题 = 当前活动标签 —— 一个对话在后台跑完时你多半正看着别的标签, 于是十几个对话的
标题全被记成了当时最活跃那个对话的, **点谁的"切过去"都落到同一个标签上**(实测症状)。
两步修法:
- hook 只在 **UserPromptSubmit / SessionStart**(用户显式操作、前台就是它本人的时刻)记窗口信息;
- 轮转要的标题**现场直接问那个 claude 的控制台**(`AttachConsole` + `GetConsoleTitleW`,
  起一次性子进程去问, ~100ms) —— 谁的控制台谁答话, 不存在张冠李戴。重名检测同源。
  注意读到的标题可能正处在被改写的瞬间(实测读到过半截), 失败重试一次就好。
...
- **永远不用窗口标题当 `taskkill` 的过滤条件。** 那个过滤器在 Win10+ 上会静默失效并杀光同名
  进程 —— 本文档后面记着这笔学费。
- **杀完人要打扫**: claude(TUI)开着终端的鼠标上报/括号粘贴/备用屏, 它正常退出会自己关,
  被 terminate 就来不及了 —— 留下的 shell 提示符上鼠标一动就满屏 `[555;170;45M` 这种
  上报序列(实测截图)。所以只杀 claude 而留 shell 时, 会 AttachConsole 到那个 shell 往
  CONOUT$ 写一串 VT 关闭序列(ConPTY 会转发给 WT/VS Code 前端, 模式状态存在前端那侧);
  返回里的 `console_reset` 说明打扫成没成。
