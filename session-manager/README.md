...
各功能表现不一样, 逐条说清楚(**下面标了「实测」的是本机验过的; 其余是按代码路径推的,
本机当时没开 VS Code, 未实测**):

| 功能 | 在 VS Code 里 | 为什么 |
|---|---|---|
| 实时状态徽章 | **照常** | 它只看 claude 进程的 pid, 与终端是谁无关(实测口径) |
| 多窗口告警 | **照常** | 同上, 靠 pid 不靠窗口 |
| `✕ 关闭` 结束对话 | **照常** | 杀的是那棵 pid 树 |
| `✕ 关闭` 顺手关窗 | **不会做** | 见下 |
| `⇥ 切过去` | **能切到 VS Code 窗口, 但切不到具体那个终端标签页** | 一个窗口多个标签共用一个 HWND |
